...
$(function() {
    ...
    $('#basket-item-table tbody').bind('DOMSubtreeModified', function(){
      var tbody = this;
      if($(tbody).children('tr').length == 0){
        $(tbody).parents('div.flex').html('<div class="bx-sbb-empty-cart-container"><div class="bx-sbb-empty-cart-image"></div><div class="bx-sbb-empty-cart-text">Ваша корзина пуста</div><div class="bx-sbb-empty-cart-desc"><a href="/">Нажмите здесь</a>, чтобы продолжить покупки</div></div>');
      }
    });
    ...
    $(".remove-basket").click(function() {
        ...
        $(".basket_items_count").html(
            '<div class="text" style="background: #f8f8f8;"><div class="name" style="font-size: 24px;margin-bottom: 15px;">В корзине</div><pstyle="color: #616161;margin-bottom: 15px;">' + basket_items_count + " товара</p></div>"
        );
    });
    ...
    $(".button-user-login").click(function() {
        if (!$(this).attr("href")) $(".popup.user").css("display", "block");
    });
